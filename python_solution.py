...
musql_password = <add DB password>
...
for url in content:
    try:
        ...
        is_status_updated = execute_mysql_query(is_status_updated, mysql_host,mysql_db_user, musql_password, None)
        ...
        if (not is_status_updated) or (is_status_updated[0][0]) < datetime.utcnow() + timedelta(minutes=-30):
            ...
            execute_mysql_query(update_db, mysql_host, mysql_db_user, musql_password, 1)
    except Exception as e:
        raise e
